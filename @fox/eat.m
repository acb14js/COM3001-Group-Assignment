...
%eaten = 1 if fox successfully finds a elk, =0 otherwise

%SUMMARY OF FOX EAT RULE
%Fox calculates distance to all elks
%Fox identifies nearest elks(s)
%If more than one equidistant within search radius, one is randomly picked
%Probability of fox killing elk =1 - distance of elk/speed of fox
%If probability > rand, fox moves to elk location and elk is
%killed
%If fox does not kill elk, it's food is decremented by one unit
...
if d<=spd&length(nrst)>0    %if there is at least one  elk within the search radius        
    ...
    pk=1-(d/spd);                       %probability that fox will kill elk is ratio of speed to distance
    if pk>rand
